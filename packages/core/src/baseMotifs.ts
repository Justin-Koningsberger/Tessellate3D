...
const rawBaseMotifs: Record<string, any> = {
  ...
  polyhedrons: (ctx: MotifContext): Point2D[][] => {
    const scale = ctx.cellHeight / 2.0;
    const state = PRESET_STATES.polyhedrons!() as SquareEditorState & { activeDetailStroke: Point2D[][] };
    state.v4.y = ctx.cellHeight;
    state.edgeTop = state.edgeTop.map((p: Point2D) => ({ x: p.x * scale, y: p.y * scale }));
    state.edgeLeft = state.edgeLeft.map((p: Point2D) => ({ x: p.x * scale, y: p.y * scale }));
    const rawTile = compileSymmetricTile(state);
    if (state.activeDetailStroke) {
      const scaledDetails = state.activeDetailStroke.map(stroke => stroke.map(p => ({ x: p.x * scale, y: p.y * scale })));
      rawTile.push(...scaledDetails);
    }
    return normalizeWorkspaceTile(rawTile, state as any, ctx.cellHeight);
  },

  dragon: (ctx: MotifContext): Point2D[][] => {
    const state = PRESET_STATES.dragon!() as TriangularEditorState & { activeDetailStroke?: Point2D[][] };

    const originY = state.v1?.y ?? 0.0;
    const workspaceHeight = state.v4.y - originY;
    const scale = ctx.cellHeight / workspaceHeight;

    state.v2 = { x: state.v2.x * scale, y: state.v2.y * scale };
    state.v4 = { x: state.v4.x * scale, y: state.v4.y * scale };

    state.edgeSpine = state.edgeSpine.map((p: Point2D) => ({ x: p.x * scale, y: p.y * scale }));
    state.edgeInterlock = state.edgeInterlock.map((p: Point2D) => ({ x: p.x * scale, y: p.y * scale }));

    const rawTile = compileSymmetricTile(state as any);

    if (state.activeDetailStroke) {
      const scaledDetails = state.activeDetailStroke.map((stroke: Point2D[]) =>
        stroke.map((p: Point2D) => ({ x: p.x * scale, y: p.y * scale }))
      );
      rawTile.push(...scaledDetails);
    }
...
// TODO: Move preset states to separate files
export const PRESET_STATES: Record<string, () => ModularEditorState> = {
  ...
  polyhedrons: (): ModularEditorState => ({
    latticeType: "square",
    v1: { x: 0, y: 0 },
    v4: { x: 0, y: 2 },
    edgeTop: [
      { x: 0.382, y: -0.179 },
      { x: 1.228, y: -0.173 },
      { x: 0.7986798679867987, y: 0.11056105610561057 },
      { x: 1.252, y: -0.167 },
      { x: 1.372937293729373, y: -0.4735973597359736 },
      { x: 1.5907590759075907, y: 0.0016501650165016502 }
    ],
    edgeLeft: [
      { x: -0.206, y: 0.103 },
      { x: -0.23, y: 1.003 },
    ...
    activeDetailStroke: [
      [
        { x: 1.1815181518151816, y: 0.018151815181518153 },
        { x: 0.9900990099009901, y: 0.5792079207920792 },
        { x: 1.603960396039604, y: 0.6056105610561056 },
        { x: 1.7095709570957096, y: 0.32838283828382836 },
        { x: 1.5907590759075907, y: 0.0049504950495049506 }
      ],
      [
        { x: 1.5907590759075907, y: 0.5858085808580858 },
        { x: 1.363036303630363, y: -0.4405940594059406 }
      ],
      [
        { x: -0.182, y: 0.118 },
        { x: 0.802, y: 0.124 },
      ...
      [
        { x: 0.1848184818481848, y: 1.3943894389438944 },
        { x: 0.16831683168316833, y: 1.8993399339933994 }
      ],
      [
        { x: 0.1782178217821782, y: 1.3778877887788779 },
        { x: 1.4653465346534653, y: 1.3745874587458746 }
      ],
      [
        { x: 1.4554455445544554, y: 1.3712871287128714 },
        { x: 1.4653465346534653, y: 1.7277227722772277 }
      ],
      [
        { x: 0.1782178217821782, y: 1.3745874587458746 },
        { x: 0.48514851485148514, y: 1.1501650165016502 }
      ],
      [
        { x: 0.48184818481848185, y: 1.1435643564356435 },
        { x: 1.6633663366336633, y: 1.1501650165016502 },
        { x: 1.438943894389439, y: 1.3745874587458746 }
      ],
      [
        { x: 1.6501650165016502, y: 1.655115511551155 },
        { x: 1.646864686468647, y: 1.146864686468647 }
      ],
      [
        { x: 1.6402640264026402, y: 1.6518151815181519 },
        { x: 1.5247524752475248, y: 1.8564356435643565 }
      ]
    ]
  }),
  dragon: (): ModularEditorState => {
    const expand = (coords: [number, number][]) => coords.map(([x, y]) => ({ x, y }));

    return {
      latticeType: "triangular",
      v1: { x: 0, y: 0 },
      v2: { x: 1.7320508075688772, y: 1 },
      v4: { x: 0, y: 2 },
      edgeSpine: expand([
        [0.3100235365, 0.1138613861], [0.4090334375, 0.1435643564], [0.5212446586, 0.1699669967],
        [0.6235548897, 0.1765676568], [0.7033502692, 0.1570000000], [0.7819707312, 0.1303630363],
        [0.8545779920, 0.0973597360], [0.9172842626, 0.0775577558], [0.9613502692, 0.0790000000],
        [1.0195944936, 0.1006600660], [1.0823007642, 0.1567656766], [1.1252050547, 0.1963696370],
        [1.1912116553, 0.2194719472], [1.2433502692, 0.2470000000], [1.2836208963, 0.2623762376],
        [1.3133238665, 0.2854785479], [1.3199245266, 0.3283828383], [1.2968222164, 0.3679867987],
        [1.3273502692, 0.4210000000], [1.3892314573, 0.4735973597], [1.4413502692, 0.5230000000],
        [1.4684393781, 0.6089108911], [1.4783403682, 0.6782178218], [1.5608486190, 0.6749174917],
        [1.6466571999, 0.6947194720], [1.6961621504, 0.7673267327], [1.7533502692, 0.9010000000]
      ]),
      edgeInterlock: expand([
        [0.1153502692, 0.1750000000], [-0.1186497308, 0.1629990000], [-0.0166497308, 0.3310000000],
        [-0.2626497308, 0.3070000000], [-0.1726497308, 0.5050000000], [-0.0766497308, 0.5950000000],
        [0.0426968038, 0.6683168317], [0.1219047246, 0.7145214521], [0.1549080250, 0.7475247525],
        [0.1813106652, 0.7871287129], [0.1219047246, 0.8102310231], [0.0327958137, 0.8399339934],
        [-0.0563130971, 0.8498349835], [-0.1586233282, 0.8597359736], [-0.2312305889, 0.8993399340],
        [-0.2840358694, 0.9554455446], [-0.2048279486, 0.9224422442], [-0.1586233282, 0.8960396039],
        [-0.0530127671, 0.8828382838], [0.0558981240, 0.8762376238], [0.1318057147, 0.8465346535],
        [0.2011126454, 0.8168316832], [0.1879113253, 0.8762376238], [0.1417067048, 0.9290429043],
        [0.1021027444, 0.9620462046], [0.0426968038, 0.9917491749]
      ]),
      activeDetailStroke: [
        expand([[1.4651390481, 0.6848184818], [1.4354360778, 0.7277227723], [1.4090334375, 0.7838283828], [1.4090334375, 0.8300330033], [1.4222347576, 0.8762376238], [1.4255350877, 0.9125412541]]),
        expand([[1.3991324474, 0.8234323432], [1.4486373979, 0.8168316831], [1.4849410283, 0.8036303631], [1.5377463088, 0.7937293729], [1.5740499392, 0.8036303631], [1.6169542296, 0.8234323432], [1.6565581900, 0.8564356436], [1.6961621504, 0.9092409241], [1.6994624804, 0.9455445545], [1.7093634705, 0.9653465347]]),
        expand([[ 1.1976388789, 0.3351863446 ], [ 1.1976388789, 0.3582886548 ], [ 1.2075398690, 0.3648893149 ], [ 1.2240415191, 0.3780906350 ], [ 1.2405431693, 0.3747903050 ], [ 1.2537444894, 0.3648893149 ], [ 1.2768467997, 0.3285856845 ], [ 1.2768467997, 0.3153843644 ], [ 1.2372428393, 0.3153843644 ], [ 1.1976388789, 0.3252853545 ], [ 1.1448335983, 0.3351863446 ], [ 1.1151306280, 0.3648893149 ], [ 1.0788269977, 0.4011929452 ]]),
        expand([[ 1.0790004342, 0.6386138614 ], [ 0.9832908632, 0.6485148515 ], [ 0.9238849227, 0.6650165017 ], [ 0.8776803022, 0.6947194720 ], [ 0.8149740316, 0.7574257426 ], [ 0.7753700712, 0.8102310231 ], [ 0.6928618203, 0.8333333333 ], [ 0.6037529095, 0.8366336634 ], [ 0.5509476289, 0.8531353135 ], [ 0.5971522494, 0.8630363036 ], [ 0.6928618203, 0.8498349835 ], [ 0.7819707312, 0.8267326733 ], [ 0.8281753517, 0.7706270627 ], [ 0.8776803022, 0.7079207921 ], [ 0.9238849227, 0.6749174917 ], [ 0.9799905332, 0.6518151815 ], [ 1.0393964738, 0.6386138614 ]]),
        expand([[ 0.4420367378, 0.8465346535 ], [ 0.3826307972, 0.8234323432 ], [ 0.2770202362, 0.8003300330 ], [ 0.2275152857, 0.7805280528 ], [ 0.1780103352, 0.7838283828 ], [ 0.1186043946, 0.8135313531 ], [ 0.0294954837, 0.8498349835 ], [ -0.0563130971, 0.8597359736 ], [ -0.1652239882, 0.8696369637 ], [ -0.2312305889, 0.9026402640 ], [ -0.2741348793, 0.9389438944 ], [ -0.2114286087, 0.9158415842 ], [ -0.1586233282, 0.8861386139 ], [ -0.0563130971, 0.8762376238 ], [ 0.0525977939, 0.8597359735 ], [ 0.1219047246, 0.8267326733 ], [ 0.1813106652, 0.8102310231 ], [ 0.2341159458, 0.7970297030 ], [ 0.2803205662, 0.8168316832 ], [ 0.3661291471, 0.8399339934 ], [ 0.4189344276, 0.8630363036 ], [ 0.4387364078, 0.8465346535 ]]),
        expand([[ 0.5080433385, 0.4504950495 ], [ 0.4684393781, 0.3613861386 ], [ 0.3628288170, 0.3943894389 ], [ 0.3166241966, 0.2821782178 ], [ 0.2209146256, 0.2986798680 ], [ 0.1978123154, 0.1963696370 ], [ 0.1384063748, 0.2095709571 ], [ 0.1186043946, 0.1171617162 ], [ 0.0657991141, 0.0907590759 ]]),
        expand([[ 0.4222347576, 0.7442244224 ], [ 0.3628288170, 0.6089108911 ], [ 0.2704195761, 0.6386138614 ], [ 0.2143139656, 0.5231023102 ], [ 0.1615086850, 0.5363036304 ], [ 0.1153040646, 0.4504950495 ], [ 0.0757001042, 0.4504950495 ], [ 0.0591984540, 0.3547854785 ], [ 0.0129938335, 0.3613861386 ], [ 0.0195944936, 0.2920792079 ], [ -0.0563130971, 0.2557755776 ]])
      ]
    };
  },
};
...
rawBaseMotifs.polyhedrons.latticeType = 'square'; rawBaseMotifs.polyhedrons.symmetryGroup = 'p1';
...
// Triangular Framework Layouts
rawBaseMotifs.dragon.latticeType          = 'triangular';
rawBaseMotifs.dragon.symmetryGroup        = 'p6';
